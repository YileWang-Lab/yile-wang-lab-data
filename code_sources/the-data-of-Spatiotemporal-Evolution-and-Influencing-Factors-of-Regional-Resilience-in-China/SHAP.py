...
National version:
- only output the core SHAP figure
- SHAP beeswarm + right-side bar
- use variable abbreviations
"""
...
OUTPUT_DIR = Path("ML_SHAP_NATIONAL")
...
plot_shap_beeswarm_bar(
    ...
    X_df=X,
    shap_df=shap_df,
    output_path_no_suffix=OUTPUT_DIR / "SHAP_Beeswarm_Bar_National",
    title_text="SHAP Summary (National)"
)
...
# 导出结果
# ======================
with pd.ExcelWriter(OUTPUT_DIR / "ML_SHAP_National_Result.xlsx", engine="openpyxl") as writer:
    metrics_df.to_excel(writer, sheet_name="Model_Metrics", index=False)
    importance_df.to_excel(writer, sheet_name="Feature_Importance", index=False)
    shap_df.to_excel(writer, sheet_name="SHAP_Values", index=False)

importance_df.to_excel(OUTPUT_DIR / "Feature_Importance.xlsx", index=False)
shap_df.to_excel(OUTPUT_DIR / "SHAP_Values.xlsx", index=False)
...
# ======================
print("=" * 70)
print("National CatBoost-XGBoost + SHAP finished.")
print(f"Input file: {Path(DATA_PATH).resolve()}")
print(f"Output directory: {OUTPUT_DIR.resolve()}")
print("Fixed parameters:")
print(FIXED_PARAMS)
print("Generated files:")
print("1. ML_SHAP_National_Result.xlsx")
print("2. Feature_Importance.xlsx")
print("3. SHAP_Values.xlsx")
print("4. SHAP_Beeswarm_Bar_National.png / .pdf")
print("=" * 70)
print("\nModel metrics:")
print(metrics_df)
