...
OUTPUT_DIR = Path("ML_SHAP_BY_ZONE")
...
# 读取数据
# ======================
required_cols = FEATURE_COLS + [TARGET_COL, "ZONE"]
df = read_excel_auto(DATA_PATH, sheet_name=SHEET_NAME, required_cols=required_cols)

keep_cols = [c for c in ID_COLS if c in df.columns] + FEATURE_COLS + [TARGET_COL]
...
with pd.ExcelWriter(OUTPUT_DIR / "ML_SHAP_By_Zone_Result.xlsx", engine="openpyxl") as writer:

    for zone_name in TARGET_ZONES:
        zone_display = ZONE_NAME_MAP.get(zone_name, zone_name)

        print("=" * 70)
        print(f"正在处理区域: {zone_display}")

        # 这里仍然按中文筛选Excel里的数据
        sub_df = df[df["ZONE"] == zone_name].copy()

        if sub_df.empty:
            print(f"[Skip] 区域 {zone_display} 没有数据。")
            continue

        sub_df = sub_df.dropna(subset=FEATURE_COLS + [TARGET_COL]).reset_index(drop=True)

        if len(sub_df) < 10:
            print(f"[Skip] 区域 {zone_display} 样本太少。")
            continue
        ...
        # 文件名和标题改成英文
        zone_safe = sanitize_name(zone_display)

        plot_shap_beeswarm_bar(
            shap_values_arr=shap_values,
            X_df=X,
            shap_df=shap_df,
            output_path_no_suffix=OUTPUT_DIR / f"SHAP_Beeswarm_Bar_{zone_safe}",
            title_text=f"SHAP Summary ({zone_display})"
        )

        # Excel sheet 名还是建议英文安全名
        metrics_df.to_excel(writer, sheet_name=f"M_{zone_safe}"[:31], index=False)
        importance_df.to_excel(writer, sheet_name=f"I_{zone_safe}"[:31], index=False)
        shap_df.to_excel(writer, sheet_name=f"S_{zone_safe}"[:31], index=False)

all_metrics_df = pd.concat(all_metrics, axis=0, ignore_index=True) if len(all_metrics) > 0 else pd.DataFrame()

with pd.ExcelWriter(OUTPUT_DIR / "ML_SHAP_By_Zone_Metrics.xlsx", engine="openpyxl") as writer:
    all_metrics_df.to_excel(writer, sheet_name="ALL_METRICS", index=False)

print("=" * 70)
print("分区域 CatBoost-XGBoost + SHAP 完成。")
print(f"输入文件: {Path(DATA_PATH).resolve()}")
print(f"输出目录: {OUTPUT_DIR.resolve()}")
print(f"区域列表: {TARGET_ZONES}")
print("输出内容：")
print("1. 中部/西部/南部各自的核心 SHAP 图（png/pdf）")
print("2. ML_SHAP_By_Zone_Result.xlsx")
print("3. ML_SHAP_By_Zone_Metrics.xlsx")
print("=" * 70)
