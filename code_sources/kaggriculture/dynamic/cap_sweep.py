...
def variants(base):
    ...
    # The shadow price is measured at 2.00 +- 0.19 early against 0.92 +- 0.08
    # late (t=+5.29 on the difference). strength scales how much of that
    # measured gap is applied: 0 = myopic, 1 = the full measured value.
    return [
        ("agent4 myopic (base)", V()),
        ("discount 0.25", V(CAPITAL_DISCOUNT=0.25)),
        ("discount 0.5", V(CAPITAL_DISCOUNT=0.5)),
        ("discount 1.0 (measured)", V(CAPITAL_DISCOUNT=1.0)),
        ("discount 1.5", V(CAPITAL_DISCOUNT=1.5)),
    ]
