...
# WHICH CHECKPOINT IS THE BASELINE IS PART OF THE EXPERIMENT. A "confirmation"
# run once loaded best_genome.json while the run it was confirming had loaded
# best_genome3.json, so the same parameter values were scored against two
# different references and neither direction meant anything. Set it explicitly
# and print it.
BASE_CKPT = os.environ.get("BASE_CKPT", "best_genome3.json")


def base_genome():
    from route.search import to_params
    path = os.path.join(ROOT, "dynamic", BASE_CKPT)
    g = json.load(open(path))["genome"]
    p = to_params(dict(g))
    p["OPP_MODEL"] = 1          # the MV rule needs the per-turn observation
    print(f"baseline checkpoint: {BASE_CKPT}", flush=True)
    return p
...
def variants(base):
    ...
    # The four that cleared win-rate t>=2.0 out of 94 in coord_descent. Max of
    # 94 noise draws sits near 2.6 sigma, so t=2.67 is at the ceiling and only
    # t=6.58 / 4.33 are clearly above it. Fresh seeds, and the pair straddling
    # each winner so a null shows as symmetry.
    return [
        ("reference (base)", V()),
        ("SHED_PANIC 0.40", V(SHED_PANIC_FRACTION=0.40)),
        ("SHED_PANIC 0.30", V(SHED_PANIC_FRACTION=0.30)),
        ("SHED_PANIC 0.55", V(SHED_PANIC_FRACTION=0.55)),
        ("IDLE_TOPUP 0", V(IDLE_TOPUP=0)),
        ("ALLOC_LABOR 16", V(ALLOC_LABOR=16.0)),
        ("PANIC .40 + TOPUP 0", V(SHED_PANIC_FRACTION=0.40, IDLE_TOPUP=0)),
    ]
