...
def variants(base):
    ...
    # 0.30 / 0.40 / 0.55 all beat the baseline value strongly on win rate, so
    # the baseline value is specifically bad rather than the direction mattering.
    # Scan the range on a fresh seed set to find where it actually sits.
    return [("reference (base)", V())] + [
        (f"PANIC {x:.2f}", V(SHED_PANIC_FRACTION=x))
        for x in (0.25, 0.35, 0.45, 0.60, 0.75, 0.90)
    ]
