...
def variants(base):
    ...
    # The tape spends $1,030 of its opening on SEED and reaches 23 tiles by day
    # 1; we spend $200 on seed, $2,400 on six animals, and reach 2.
    return [
        ("current (animals first)", V()),
        ("animal cap 0.7", V(OPENING_ANIMAL_CAP=0.7)),
        ("animal cap 0.5", V(OPENING_ANIMAL_CAP=0.5)),
        ("animal cap 0.35", V(OPENING_ANIMAL_CAP=0.35)),
        ("animal cap 0.5, 5d", V(OPENING_ANIMAL_CAP=0.5, OPENING_DAYS=5)),
        ("animal cap 0.35, 5d", V(OPENING_ANIMAL_CAP=0.35, OPENING_DAYS=5)),
        ("cap 0.5 + batch16", V(OPENING_ANIMAL_CAP=0.5, SEED_BATCH_PER_TURN=16)),
    ]
