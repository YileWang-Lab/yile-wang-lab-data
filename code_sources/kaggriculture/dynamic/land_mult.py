...
def variants(base):
    ...
    # LAND_BUY_CASH_MULTIPLE was the only survivor of the champion's five
    # changes (65.5%, t=2.29). It was also the argmax of those five, so it
    # carries its own selection bias and needs a fresh seed set -- and a scan,
    # to tell a plateau from a spike. enpv_land says the first quadrant is worth
    # +18,110 at day 0, so lower (buy earlier) should be better.
    return [
        ("reference (base)", V()),
        ("LBCM 1.02", V(LAND_BUY_CASH_MULTIPLE=1.02)),
        ("LBCM 1.10", V(LAND_BUY_CASH_MULTIPLE=1.10)),
        ("LBCM 1.25", V(LAND_BUY_CASH_MULTIPLE=1.25)),
        ("LBCM 1.45", V(LAND_BUY_CASH_MULTIPLE=1.45)),
        ("LBCM 2.00 (worse side)", V(LAND_BUY_CASH_MULTIPLE=2.00)),
    ]
