...
def variants(base):
    def V(**kw):
        ...
        p.update(kw)
        return p
    # Variants of KNOWN effect size, so the two metrics can be compared against
    # ground truth: an exact null, three confirmed effects of different size,
    # and one known-bad.
    return [
        ("reference (base)", V()),
        ("exact null (identity)", V(ENPV_VETO=1)),
        ("late wheat off", V(NURSE_LATE=0, ALLOC_MODE=0)),
        ("veto off", V(ENPV_VETO=0)),
        ("alloc off", V(ALLOC_MODE=0)),
        ("veto L20 (known bad)", V(ENPV_LABOR=20.0)),
    ]
