...
def _agent(kind):
    ...
    if kind.startswith("tree"):
        ...
        _n[0] += 1
        spec = importlib.util.spec_from_file_location(
            f"tp_{os.getpid()}_{_n[0]}",
            os.path.join(ROOT, "dynamic", "rl", "agent_rl.py"))
...
def main():
    ...
    cands = [("identity (scheduler)", "identity"),
             ("tree blend 0.5", "tree:0.5"),
             ("tree blend 1.0", "tree:1.0"),
             ("tree blend 2.0", "tree:2.0"),
