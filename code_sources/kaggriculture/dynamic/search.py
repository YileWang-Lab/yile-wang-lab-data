...
CKPT = os.path.join(ROOT, "dynamic", "best_genome.json")
LOG_DIR = os.path.join(ROOT, "logs", "dynamic_optimization")
...
def main():
    ...
    seed_genome = json.load(open(os.path.join(ROOT, "planner", "checkpoints",
                                              "best_genome1.json")))["genome"]
    base = to_params(dict(seed_genome)); base["SCHEDULE_DRIVEN"] = 0
    ...
    for gen in range(args.generations):
        ...
        with open(PROGRESS, "w") as f:
            f.write(f"# dynamic/ GA progress\n\n{time.strftime('%Y-%m-%d %H:%M:%S')}\n\n"
                    ...
                    f"- reference: tape+market build **+17,235**, "
                    f"route/agent.py start **-78,027**\n"
                    f"- generations since improvement: {stale}/{args.patience}\n\n"
                    f"Searching the cash-flow split (hire budget, reserves, purchase\n"
                    f"throttles, feed buffer, land threshold), not size -- six scaling\n"
                    f"hypotheses were refuted and all pointed at capital and labour\n"
