"""agent_rl = agent4 with its four decision points routed through a Policy.

Generated from agent4.py. With `POLICY = None` -- or with a ScriptedPolicy --
this file must reproduce agent4 EXACTLY; that identity is the control for the
whole RL line and is asserted by dynamic/rl/test_identity.py. If it ever fails,
the interface is changing behaviour on its own and every later comparison is
worthless.

The four hooks, in order of how much volume they govern:

  sell_level[item]   scales the quantity offered. The current code dumps
                     everything whenever the shed passes a threshold, and that
                     one constant governs 79% of all units we sell.
  crop_pref[crop]    multiplies ENPV in the opportunity allocator, so the
                     policy re-weights a ranking rather than replacing it.
  crew_delta         shifts the crew off the scheduler's own estimate.
  buy_land / animal  the two lumpy capital decisions.

agent4 = agent3 + the zero-drag cash policy.

A COPY again: dynamic/search3.py is running a GA over agent3.py and its workers
re-exec that file per evaluation. agent4 with ZERO_DRAG=0 is agent3 exactly.
...
# ZERO-DRAG CASH. Measured, seed 1009, from day 12 to the buzzer:
#
#     mean idle cash $22,900, final $49,562, and FIFTY TILES STILL LOCKED
#
# The deadlock is the land guard. `wanted` requires a non-EMPTY role in the next
# quadrant, but the layout only holds as many roles as TARGET_COUNTS sums to --
# 49, which is exactly NW+NE -- so SW never holds a role, `wanted` is
# permanently False, the quadrant is never bought, its tiles never become
# available, and no role is ever assigned to them. Cash then piles up with
# nothing to buy.
#
#     Loss_drag(t) = max(0, C_t - C_reserve - min cost) * r_marginal
#
# Every previous expansion attempt spent cash the RAMP needed (proportional
# scale-up -87,245, the 73-tile plan -62,148, TC_MELON 12 -21,056). This spends
# only cash that is provably idle, from day 12 onward, after the ramp is over --
# which makes it additive rather than a reallocation, the shape that has held.
# `dynamic/spatial.py` says 12 workers can service 80 crop tiles against our 49,
# so the crew has the headroom.
# The policy object. None means "behave exactly like agent4".
POLICY = None
_PDEC = [None]          # the current day's Decision
...
def _dec():
    """Today's Decision, or None when running unpolicied."""
    return _PDEC[0]


ZERO_DRAG = 0
DRAG_MIN_IDLE = 1500.0   # idle cash above the reserve before expanding
DRAG_ADD_PER_DAY = 4     # tiles given a role per day, so seed spend is paced
DRAG_MAX_TILES = 74      # spatial ceiling; never claim more than we can water
DRAG_LAND_MULT = 1.3     # buy the next quadrant at this multiple of its price
# The v1/v2 trigger fired on day 12 with $3,538 -- squarely inside the build-out,
# not after it -- and took $2,000 of it for land. That is the same mistake as
...
def _plan_day(farm, private, day, board_size):
    ...
    d = _dec()
    if d is not None and d.crew_delta:
        n_units = max(1, min(int(MAX_HANDS) + 1, n_units + d.crew_delta))
...
def _alloc_role(role, day):
    ...
    if d is not None:
        ...
        def price_of(crop, _r=_raw, _w=w):        # noqa: F811
            return _r(crop) * float(_w.get(crop, 1.0))
...
def _zero_drag_expand(farm, private, day):
    """Give roles to EMPTY unlocked tiles while cash sits idle above reserve.

    Only ever ADDS roles, never moves or removes one, so the searched layout is
    untouched wherever it has an opinion.
    """
    ctx = S.get("econ")
    if ctx is None:
        return
    ...
    shed = private.get("shed") or {}
    reserve = EN.reserve_cash(day, _placed_animals(farm),
                              S.get("target_units", 1),
                              ctx.market_price("WHEAT"),
                              dry_days=ENPV_DRY_DAYS, floor=SPEND_RESERVE)
    idle = money - reserve
    if idle < DRAG_MIN_IDLE or day < int(DRAG_START_DAY):
        return
    claimed = [pos for pos, r in S["layout"].items()
               if r != "EMPTY" and _unlocked(farm, pos)]
    if len(claimed) >= int(DRAG_MAX_TILES):
        return
    free = [pos for pos, r in S["layout"].items()
            if r == "EMPTY" and _unlocked(farm, pos)
            and farm["tiles"][pos[1]][pos[0]] is None]
    free.sort(key=lambda t: (dist_to_shed(t), t))

    # Each new tile must be priced against the ones just added, not against a
    # single marginal unit. The first version chose by `ctx.unit_price`, which
    # quotes ONE more unit, and then planted 25 tiles of the argmax -- MELON,
    # whose book has no town drain at all and 158 units to the floor. Twenty-five
    # melon tiles is ~150 melons: it floors the book and destroys the revenue of
    # the melon we already had. Measured -37,683.
    #
    # So the running supply is carried through the loop and the tile is only
    # claimed while its ENPV is still positive given everything ahead of it.
    pending = {}
    added = 0
    for pos in free:
        if added >= int(DRAG_ADD_PER_DAY) or len(claimed) + added >= int(DRAG_MAX_TILES):
            break
        best_crop, best_v = None, 0.0
        for c in OPP.feasible(day):
            n_pend = _pending_units(farm, c, day) + pending.get(c, 0.0)
            v, det = EN.enpv_crop(c, day, ctx, c_labor=ALLOC_LABOR,
                                  n_pending=n_pend)
            if v > best_v:
                best_crop, best_v = c, v
        if best_crop is None:
            break
        if idle < CROPS[best_crop]["seed"] * 2:
            break
        units, _, _ = OPP.yield_plan(best_crop, day)
        pending[best_crop] = pending.get(best_crop, 0.0) + units
        S["layout"][pos] = best_crop
        idle -= CROPS[best_crop]["seed"]
        added += 1
...
def _market_orders(farm, private, day, hour, prices, shops=(), opp_farm=None,
    ...
    if len(orders) < MAX_ORDERS:
        ...
        if n_extra < len(LAND_PRICES):
            ...
            if SCHEDULE_DRIVEN:
                ...
                if ZERO_DRAG and not wanted and day >= int(DRAG_START_DAY):
                    # The guard is circular: no role is assigned in a quadrant
                    # we have not bought, and we never buy one nothing is
                    # assigned in. Idle cash breaks the tie.
                    wanted = money >= price * DRAG_LAND_MULT + SPEND_RESERVE
                if wanted and money >= price * min(LAND_BUY_CASH_MULTIPLE,
                                                   DRAG_LAND_MULT if ZERO_DRAG
                                                   else LAND_BUY_CASH_MULTIPLE):
                    orders.append(["BUY_LAND"])
                    money -= price
    ...
    for item in order:
        ...
        if value >= reserve or panic:
            d = _dec()
            if d is not None:
                frac = float(d.sell_level.get(item, 1.0))
                qty = int(qty * frac) if frac < 1.0 else qty
                if qty <= 0:
                    continue
...
def agent(obs):
    ...
    if ZERO_DRAG and S.get("econ") is not None:
        _zero_drag_expand(farm, private, day)
