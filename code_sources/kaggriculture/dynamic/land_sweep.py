...
def variants(base):
    ...
    import os as _os
    # The agent buys a quadrant of its own accord and that purchase has never
    # been priced. This only DECLINES it when the tiles cannot repay the land.
    return [
        ("agent3 (base)", _os.path.join(ROOT, "dynamic", "agent3.py")),
        ("agent4 off (identity)", V()),
        ("land veto, usable 60%", V(LAND_ENPV_VETO=1)),
        ("land veto, usable 40%", V(LAND_ENPV_VETO=1, LAND_USABLE_FRAC=0.4)),
        ("land veto, usable 80%", V(LAND_ENPV_VETO=1, LAND_USABLE_FRAC=0.8)),
        ("land veto, usable 100%", V(LAND_ENPV_VETO=1, LAND_USABLE_FRAC=1.0)),
    ]
