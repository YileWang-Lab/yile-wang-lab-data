...
def variants(base):
    ...
    import os as _os
    # v3: gate on the DAY, which is what "only idle cash" was supposed to mean.
    Z = dict(ZERO_DRAG=1)
    return [
        ("agent3 (base)", _os.path.join(ROOT, "dynamic", "agent3.py")),
        ("agent4 off (identity)", V()),
        ("zd from d14", V(DRAG_START_DAY=14, **Z)),
        ("zd from d17", V(DRAG_START_DAY=17, **Z)),
        ("zd from d20", V(DRAG_START_DAY=20, **Z)),
        ("zd from d17, idle 8k", V(DRAG_START_DAY=17, DRAG_MIN_IDLE=8000.0, **Z)),
        ("zd from d17, no land", V(DRAG_START_DAY=17, DRAG_LAND_MULT=99.0, **Z)),
        ("zd from d20, 8/day", V(DRAG_START_DAY=20, DRAG_ADD_PER_DAY=8, **Z)),
    ]
