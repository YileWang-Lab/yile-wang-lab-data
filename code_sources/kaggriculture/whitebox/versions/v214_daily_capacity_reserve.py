"""V214: one-slot daily capacity reserve (research-only).

Public notebooks use a small day-close capacity guard to avoid losing carried
output when the engine auto-deposits into a full shed.  This wrapper transfers
only that state-derived mechanism: the current unit phase is projected, and an
extra SELL is emitted only when the exact projected total exceeds 99 of the
100-item shed.  No route, replay, opponent identity, or future action is read.

V204 remains the production baseline until this candidate clears the paired
promotion gate.
"""
...
def whitebox_v214_daily_capacity_reserve(obs):
    _pin_v214_runtime()
