"""C165 clone horizon 4.

This retains C164's repeated public top-10 field/market route and extends its
observation-driven clone front-run from one to four steps. The horizon was
selected on one seed block, then cleared two independent full current-top-10
blocks and a separate top-100-medoid regression gate.
"""
import base64
import copy
import json
...
# --- K304 observable competitor-pressure route ---------------------------
_K304_HOST = agent
_K304_PROFILE_BASE = _update_clone_profile
_K304_PRESSURE_STATE = {
    0: {"last": -1, "votes": 0, "samples": 0, "active": False},
    1: {"last": -1, "votes": 0, "samples": 0, "active": False},
}


def _k304_counts(farm):
    counts = {key: 0 for key in ("COW", "SHEEP", "MELON", "STRAWBERRY", "PLANT")}
    for row in farm.get("tiles", []) or []:
        for tile in row or []:
            if not isinstance(tile, dict):
                continue
...
def _k304_update_profile(obs, step):
    """Activate horizon release after two visible production-pressure votes."""
    global _CLONE_CONFIDENCE
    _K304_PROFILE_BASE(obs, step)
    player = 1 if int(obs.get("player", 0) or 0) == 1 else 0
    state = _K304_PRESSURE_STATE[player]
    if step == 0 or step <= int(state.get("last", -1)):
        state = {"last": step, "votes": 0, "samples": 0, "active": False}
        _K304_PRESSURE_STATE[player] = state
    state["last"] = step
    if step in (120, 144, 168, 192):
        farms = list(obs.get("farms", []) or [])
        rival = farms[1 - player] if len(farms) >= 2 else {}
        counts = _k304_counts(rival)
        productive = (
            counts["PLANT"] >= 15
            and (
                counts["COW"] >= 4
                or counts["SHEEP"] >= 2
                or counts["MELON"] >= 8
                or counts["STRAWBERRY"] >= 4
            )
        )
        state["samples"] += 1
        state["votes"] += int(productive)
        if state["samples"] >= 2 and state["votes"] >= 2:
            state["active"] = True
    if state.get("active"):
        _CLONE_CONFIDENCE = max(1, _CLONE_CONFIDENCE)


_update_clone_profile = _k304_update_profile


def kaggriculture_k304_agent(obs):
    return _K304_HOST(obs)


agent = kaggriculture_k304_agent
__version__ = "K304-diversified-horizon4-opponent-pressure"
