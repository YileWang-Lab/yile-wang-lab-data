...
MAX_HANDS = 16
# Crew floor during the growth phase; see _size_crew for why demand-driven
# sizing alone cannot bootstrap. 0 disables (identical to the old behaviour).
MIN_CREW = 0
MIN_CREW_UNTIL_DAY = 14
...
_GENOME_KEYS = ("MAX_HANDS", "MIN_CREW", "MIN_CREW_UNTIL_DAY",
...
def _size_crew(tasks, money, farmer_pos, day=0):
    """Smallest crew that gets through the day's tasks, capped by what the
    fib-priced hire ladder can be afforded from today's cash.

    MIN_CREW breaks a bootstrap loop that this sizing rule creates on its own.
    Sizing the crew to *today's* tasks is circular: tasks come from planted
    tiles, planted tiles come from yesterday's crew getting through its PLANT
    tasks, so a small crew produces a small task list which justifies a small
    crew. Measured over 32 traces (planner/spec_extract.py), that loop is most
    of the gap to the tape -- it hires 277 times to our 188 (0.68x) and lands
    186 PLANT and 53 PLACE ops to our 67 and 13 (0.36x / 0.25x), while our
    realised price per unit is actually BETTER than the tape's ($89.7 vs $79.6)
    and our useful-op rate is higher (44.5% vs 40.1%). We are not less
    efficient; we are running a farm about two thirds the size.

    The floor applies only through MIN_CREW_UNTIL_DAY -- once the board is
    planted out, demand-driven sizing is the right rule and over-hiring is
    just wasted fib-priced cash.
    """
    if not tasks:
        return max(1, int(MIN_CREW) if day <= MIN_CREW_UNTIL_DAY else 1)
    cash_cap = 1
    ...
    for n in range(1, cash_cap + 1):
        ...
        if not leftover:
            need = n
            break
    if day <= MIN_CREW_UNTIL_DAY:
        need = max(need, min(int(MIN_CREW), cash_cap))
    return need
...
def _market_orders(farm, private, day, hour, prices, shops=(), opp_farm=None):
    ...
    # 5. Land, once the next quadrant's tiles actually have roles waiting.
    if len(orders) < MAX_ORDERS:
...
def agent(obs):
    ...
    if S.get("layout") is None:
        S["layout"] = _build_layout(board_size)
